...
func assertEqual<T: Equatable>(_ a: T, _ b: T, _ name: String, file: String = #file, line: Int = #line) {
    if a == b {
        passed += 1
    } else {
        failed += 1
...
func buildQr(seed: Data, host: String, prefer: String) -> Data {
    ...
    buf.append(0xBC); buf.append(0x1B)
    buf.append(seed)
...
func testSeedDerivation() {
    assertEqual(BuzzelProtocol.deriveSessionId(seed), expectedSessionId, "deriveSessionId cross-platform")
    assertEqual(BuzzelProtocol.derivePairingCode(seed), expectedPairingCode, "derivePairingCode cross-platform")
...
func testPairRequest() {
    ...
    // Wrong signal
    assertNil(BuzzelProtocol.parsePairRequestCode(Data([0x99, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36])), "parsePairRequest wrong signal")

    // Too short
    assertNil(BuzzelProtocol.parsePairRequestCode(Data([Signal.pairRequest, 0x31])), "parsePairRequest too short")
}
...
func testPairResponse() {
    ...
    // Too short
    assertNil(BuzzelProtocol.parsePairResponse(Data([Signal.pairResponse, 0x01])), "parsePairResponse too short")
}
...
func testAck() {
    ...
    assertEqual(BuzzelProtocol.parseAckSeq(BuzzelProtocol.createAck(seq: 65535)), 65535, "ack max seq")
...
func testParseSignalId() {
    ...
    assertEqual(BuzzelProtocol.parseSignalId(Data([0x01])), Signal.pairRequest, "signalId pairRequest")
...
func testCommand() {
    ...
    assertEqual(BuzzelProtocol.parseCommand(trunc)!.data.count, BuzzelProtocol.maxTlvData, "command truncate")

    // Too short
    assertNil(BuzzelProtocol.parseCommand(Data([Signal.command, 0x01, 0x00])), "parseCommand too short")

    // Wrong signal
    assertNil(BuzzelProtocol.parseCommand(Data([Signal.ping, 0x01, 0x00, 0x01])), "parseCommand wrong signal")
}
...
func testWireSize() {
    ...
    // pair.request: 2 + 7 = 9
    assertEqual(FrameCodec.encode(BuzzelProtocol.createPairRequest(code: "123456")).count, 9, "wireSize pairRequest")

    // pair.response: 2 + 3 = 5
    assertEqual(FrameCodec.encode(BuzzelProtocol.createPairResponse(accepted: true)).count, 5, "wireSize pairResponse")

    // ack: 2 + 3 = 5
    assertEqual(FrameCodec.encode(BuzzelProtocol.createAck(seq: 1)).count, 5, "wireSize ack")

    // command (empty): 2 + 4 = 6
    assertEqual(FrameCodec.encode(BuzzelProtocol.createCommand(cmd: 0x01, seq: 0)).count, 6, "wireSize commandEmpty")

    // command (max TLV): exactly 256 bytes
    let tlv = Data(count: BuzzelProtocol.maxTlvData)
    assertEqual(FrameCodec.encode(BuzzelProtocol.createCommand(cmd: 0x01, seq: 0, tlvData: tlv)).count, BuzzelProtocol.maxFrame, "wireSize commandMaxTlv")
}
...
func testBigEndian() {
    ...
    // TLV int 0x01020304
    let tlvInt = BuzzelProtocol.tlvEncodeInt(tag: 0x01, value: 0x01020304)
...
func testMalformedInput() {
    ...
    assertNil(BuzzelProtocol.parseAckSeq(Data([Signal.ping, 0x00, 0x01])), "malformed ack wrong signal")
    ...
    let r0 = BuzzelProtocol.parsePairResponse(BuzzelProtocol.createPairResponse(accepted: false, reason: 0x00))
    assertEqual(r0!.reason, 0x00, "malformed pairResponse reason 0x00")
    let r1 = BuzzelProtocol.parsePairResponse(BuzzelProtocol.createPairResponse(accepted: false, reason: 0x01))
    assertEqual(r1!.reason, 0x01, "malformed pairResponse reason 0x01")
    let r2 = BuzzelProtocol.parsePairResponse(BuzzelProtocol.createPairResponse(accepted: false, reason: 0x02))
...
func testFrameCodecEdgeCases() {
    ...
    assertEqual(BuzzelProtocol.tlvGetString(cmdFields, tag: 0x01), "test", "frameEdge roundTrip string")
