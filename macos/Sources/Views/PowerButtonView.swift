...
struct PowerButtonView: View {
    ...
    private enum Timing {
        static let holdWarningDuration: TimeInterval = 2.0
        static let holdUnpairDuration: TimeInterval = 4.0
    }
    ...
    var body: some View {
        ...
        .gesture(
            DragGesture(minimumDistance: 0)
                .onChanged { _ in handlePressStarted() }
        ...
        .overlay {
            TimelineView(
                .animation(minimumInterval: 1.0 / 60.0, paused: holdStart == nil || !canUnpair)
            ) { timeline in
                Color.clear
                    .onChange(of: timeline.date) { _ in
                        guard canUnpair, let start = holdStart, !holdUnpairFired else { return }
                        let elapsed = Date().timeIntervalSince(start)
                        if elapsed >= Timing.holdUnpairDuration {
                        ...
                        } else if elapsed >= Timing.holdWarningDuration {
                            if !holdWarningFired {
                                holdWarningFired = true
                                onUnpairWarning?()
                            }
                            let progressDuration =
                                Timing.holdUnpairDuration - Timing.holdWarningDuration
                            unpairProgress =
                                (elapsed - Timing.holdWarningDuration) / progressDuration
                        }
                    }
            }
        }
    ...
    private var currentIcon: ButtonIcon {
        switch state {
    ...
    private var iconView: some View {
        ...
        GeometryReader { geo in
            ...
            let center = UnitPoint(
                x: (offset + iconSize / 2) / size, y: (offset + iconSize / 2) / size)
    ...
    private func blend(from: Color, to: Color, fraction: CGFloat) -> Color {
        ...
        fromComponents.getRed(&fromRed, green: &fromGreen, blue: &fromBlue, alpha: &fromAlpha)
        toComponents.getRed(&toRed, green: &toGreen, blue: &toBlue, alpha: &toAlpha)
        return Color(
            red: fromRed + (toRed - fromRed) * clampedFraction,
            green: fromGreen + (toGreen - fromGreen) * clampedFraction,
            blue: fromBlue + (toBlue - fromBlue) * clampedFraction,
            opacity: fromAlpha + (toAlpha - fromAlpha) * clampedFraction
        )
    }
    ...
    enum ButtonIcon: Equatable {
        ...
        func view(scale: CGFloat, offset: CGFloat) -> some View {
            ...
            switch self {
            case .fingerAccess:
                Path { path in path.addPath(parseSVGPath(Self.fingerAccessPath).applying(transform))
                }
                .fill(DesignColor.onButton)

            case .loading:
                Path { path in path.addPath(parseSVGPath(Self.loadingPath).applying(transform)) }
                    .fill(DesignColor.onButton)

            case .zap:
                Path { path in path.addPath(parseSVGPath(Self.zapPath).applying(transform)) }
                    .fill(DesignColor.onButton)

            case .unlink:
                Path { path in path.addPath(parseSVGPath(Self.unlinkPath).applying(transform)) }
                    .fill(DesignColor.onButton)

            case .play:
                Path { path in
                    path.addPath(parseSVGPath("M10 8l6 4l-6 4z").applying(transform))
                }
                .stroke(
