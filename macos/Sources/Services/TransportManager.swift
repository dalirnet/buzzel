...
class TransportManager: ObservableObject, BleCentralDelegate, TcpServerDelegate {
    ...
    private func buildFailoverSteps(prefer: String) {
        ...
        os_log("Failover steps: %{public}@", log: log, type: .debug,
    ...
    private func transitionTo(_ newState: ConnectionState) {
        ...
        os_log("State: %{public}@ → %{public}@", log: log, type: .info,
        ...
        case .idle:
            ...
            cancelRetry()
            DispatchQueue.main.async { self.isConnected = false; self.activeTransport = "" }
        case .connecting:
            DispatchQueue.main.async { self.isConnected = false; self.activeTransport = "" }
        case .handshaking:
    ...
    private func startHandshakeTimeout() {
        ...
        let work = DispatchWorkItem { [weak self] in
            ...
            self.appendEntry(.deviceDisconnected, "Handshake timeout",
    ...
    private func startKeepalive() {
        ...
        timer.setEventHandler { [weak self] in
            ...
            if let last = self.lastPongTime, Date().timeIntervalSince(last) > Self.pongTimeout {
                ...
                self.appendEntry(.deviceDisconnected, "Pong timeout",
    ...
    private func startRetryTimer() {
        ...
        let work = DispatchWorkItem { [weak self] in
            ...
            if self.pendingAckSeq != nil && self.pendingRetries < 3 {
                ...
                os_log("Retry command seq=%d, attempt=%d", log: log, type: .info,
    ...
    private func handleIncomingPairingRequest(_ payload: Data) {
        ...
        if code == pairingCode {
            ...
            DispatchQueue.main.async { [weak self] in
                ...
                self?.appendEntry(.pairingFailed, "Invalid pairing code", direction: .incoming, status: .failed)
            }
        }
    }
    ...
    private func transportDidReceiveData(_ data: Data, via transport: Transport) {
        os_log("Received %d bytes via %{public}@", log: log, type: .debug, data.count, transport.rawValue)
        handleIncoming(data)
    }
