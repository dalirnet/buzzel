...
private func addArc(
  ...
  var curAngle = theta1
  var curCos = cos(curAngle)
  var curSin = sin(curAngle)
  ...
  for _ in 0..<segments {
    ...
    let q1x = ex1 - alpha * ey1
    let q1y = ey1 + alpha * ex1
    let q2x = ex2 + alpha * ey2
    let q2y = ey2 - alpha * ex2
...
func parseSVGPath(_ d: String) -> Path {
  ...
  while i < chars.count {
    ...
    case "S":
      ...
      let c1 =
        lastControl.map { CGPoint(x: 2 * cx - $0.x, y: 2 * cy - $0.y) } ?? CGPoint(x: cx, y: cy)
      path.addCurve(to: p, control1: c1, control2: c2)
      lastControl = c2
      cx = p.x
      cy = p.y
    ...
    case "A", "a":
      ...
      guard let rx = parseNumber(), let ry = parseNumber(),
        ...
        let ex = parseNumber(), let ey = parseNumber()
      else { break }
